...
object GroupingFunction {
  ...
  def computeBuckets(cVectors: List[CVector],
                     groupingIdentifiers: String,
                     totalBuckets: Int): CodeLines = {
  ...
  def computeBucketSizes(groupingIdentifiers: String,
  ...
  def copyVecToBucketsStmt(output: CVector,
                           input: CVector,
                           buckets: Int,
                           idToBucket: String,
                           bucketToCount: String): CodeLines = {
        ...
        CodeLines.from(
          ...
          CodeLines.forLoop("i", s"${idToBucket}.size()") {
            CodeLines.ifStatement(s"b == ${idToBucket}[i]") {
              ...
              s"set_validity(${outName}->validityBuffer, o++, check_validity(${input.name}[0]->validityBuffer, i);",
            }
          }
        )
          ...
          CodeLines.forLoop("i", s"${idToBucket}.size()") {
            ...
            CodeLines.ifStatement(s"b == ${idToBucket}[i]") {
              List(
                ...
                // Preserve the validity bit values across bucketing
                s"set_validity(${output.name}[b]->validityBuffer, o++, check_validity(${input.name}[0]->validityBuffer, i);",
              )
            }
          }
        )
    ...
    CodeLines.scoped {
      CodeLines.from(
        s"// STEP: Copy elements of ${input.name}[0] to their respective buckets in ${output.name}",
  ...
  def groupDataNormal(data: List[DataDescription], totalBuckets: Int): CFunction2 = {
    ...
    CFunction2(
      ...
      body = CodeLines.from(
        ...
        (outputs, inputs).zipped.map(copyVecToBucketsStmt(_, _, totalBuckets, "idToBucket", "bucketToCount"))
      )
    )
  }
}
